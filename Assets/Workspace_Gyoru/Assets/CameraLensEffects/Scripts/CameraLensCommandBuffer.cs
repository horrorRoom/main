...
namespace Wilberforce.CameraLens
{
    ...
    public class CameraLensCommandBuffer : MonoBehaviour
    {
        ...
        public Shader cameraLensShader;

        private Camera myCamera = null;
        private bool isSupported;
        private Material CameraLensMaterial;
        ...
        public void Start()
        {
            if (cameraLensShader == null) cameraLensShader = Shader.Find("Hidden/Wilberforce/CameraLens");
            if (cameraLensShader == null)
            {
                ReportError("Could not locate Wilberforce Camera Lens Shader. Make sure there is 'CameraLens.shader' file added to the project.");
                isSupported = false;
                enabled = false;
                return;
            }
            ...
            if (!CameraLensMaterial || CameraLensMaterial.passCount != Enum.GetValues(typeof(ShaderPass)).Length)
            {
        ...
        {
            if (!isSupported || !cameraLensShader.isSupported)
            {
                enabled = false;
                return;
            }
        ...
        private void CreateCommandBuffer()
        {
            ...
            else
            {
                ...
                commandBuffer.name = "Wilberforce Camera Lens";
        ...
        {
            if (CameraLensMaterial == null) return;

            ResolveActiveEffects();

            CameraLensMaterial.SetInt("isDistortionBeforeDof", (DistortionEnabled && (DistortionOrder < DofOrder)) ? 1 : 0);

            CameraLensMaterial.SetFloat("cameraFarPlane", myCamera.farClipPlane);
            CameraLensMaterial.SetInt("useCameraFarPlane", FarPlaneSource == FarPlaneSourceType.Camera ? 1 : 0);

            CameraLensMaterial.SetInt("isImageEffectMode", IntegrationType == Integration.ImageEffect ? 1 : 0);
            CameraLensMaterial.SetInt("useSPSRFriendlyTransform", 0);
            ...
            CameraLensMaterial.SetFloat("minTexelSize", minTexelSize);
            CameraLensMaterial.SetFloat("minTexelSizeDofQuality", minTexelSize * 2.0f);

            CameraLensMaterial.SetFloat("fastBlurMixingCoeff", 1.0f / (minTexelSize * 9.0f));
            CameraLensMaterial.SetVector("dofTexelSize", texSizeDof);
            CameraLensMaterial.SetVector("dofFastPassTexelSize", texSizeDof * 2.0f);

            CameraLensMaterial.SetInt("useGBuffer", ShouldUseGBuffer() ? 1 : 0);

            // Debug
            if (VignetteDebugEnabled)
            {
                CameraLensMaterial.SetInt("debugMode", (int)DebugModeType.Vignette);
            }
            else
            {
                CameraLensMaterial.SetInt("debugMode", (int)DebugModeType.None);
            }

            // Lens parameters
            CameraLensMaterial.SetFloat("aspectRatio", aspectRatio);
            CameraLensMaterial.SetFloat("apertureDiameter", ApertureDiameter * 0.001f);
            CameraLensMaterial.SetFloat("focalPlaneDistance", -FocalPlaneDistance);
            ...
            CameraLensMaterial.SetFloat("focalLength", dofFocalLength);
            CameraLensMaterial.SetFloat("sensorHeight", SensorHeight * 0.001f);

            float cocCoefficient = ((ApertureDiameter * 0.001f) * (dofFocalLength / ((-FocalPlaneDistance) - dofFocalLength))) / (SensorHeight * 0.001f);
            CameraLensMaterial.SetFloat("cocCoefficient", cocCoefficient);
            ...
            CameraLensMaterial.SetFloat("chromaTexSize", ChromaSize * 0.02f);
            CameraLensMaterial.SetFloat("chromaWeight", ChromaWeight);
            CameraLensMaterial.SetFloat("chromaRadialness", ChromaRadialness);
            CameraLensMaterial.SetFloat("chromaQuality", chromaQualityCoeff);
            CameraLensMaterial.SetFloat("chromaSpectrumMin", ChromaSpectrumMin);
            CameraLensMaterial.SetFloat("chromaSpectrumMax", ChromaSpectrumMax);
            EnsureChromaSamplesTexture();

            CameraLensMaterial.SetVector("chromaBigWeightRcp", chromaBigWeightRcp);

            CameraLensMaterial.SetInt("isAnamorphicChroma", IsAnamorphicChroma ? 1 : 0);

            // Vignette
            CameraLensMaterial.SetFloat("vignetteFalloff", VignetteFalloff);
            CameraLensMaterial.SetFloat("vignetteMax", VignetteOuterValue);
            CameraLensMaterial.SetFloat("vignetteMin", VignetteInnerValue);
            CameraLensMaterial.SetFloat("vignetteMaxDistance", VignetteOuterValueDistance);
            CameraLensMaterial.SetFloat("vignetteMinDistance", VignetteInnerValueDistance);

            CameraLensMaterial.SetVector("vignetteCenter", VignetteCenter);
            CameraLensMaterial.SetVector("vignetteInnerColor", VignetteInnerColor);
            CameraLensMaterial.SetVector("vignetteOuterColor", VignetteOuterColor);
            CameraLensMaterial.SetFloat("vignetteSaturationMin", VignetteInnerSaturation);
            CameraLensMaterial.SetFloat("vignetteSaturationMax", VignetteOuterSaturation);
            CameraLensMaterial.SetInt("isAnamorphicVignette", IsAnamorphicVignette ? 1 : 0);
            CameraLensMaterial.SetInt("vignetteMode", (int)VignetteMode);

            // Bloom
            TrySetBloomUniforms(screenTextureWidth, screenTextureHeight);

            // Distortion
            CameraLensMaterial.SetFloat("barrelPower", DistortionPower + 1);
            CameraLensMaterial.SetVector("distortionMaxUV", GetDistortionMaxUV(aspectRatio, IsAnamorphicDistortion, DistortionPower + 1));
            CameraLensMaterial.SetInt("isAnamorphicDistortion", IsAnamorphicDistortion ? 1 : 0);

            // Depth of Field
            CameraLensMaterial.SetInt("dofRange", (int)DofRange);
            CameraLensMaterial.SetInt("dofQuality", (int)DofQuality);

            switch (DofQuality)
            {
                case Quality.High:
                    CameraLensMaterial.SetFloat("dofQualityCoeff", 1.0f);
                    break;
                case Quality.Low:
                    CameraLensMaterial.SetFloat("dofQualityCoeff", 0.75f);
                    break;
                case Quality.Ultra:
                    CameraLensMaterial.SetFloat("dofQualityCoeff", 1.5f);
                    break;
            }
            ...
            {
                CameraLensMaterial.SetVector("cocBlurTexSizeRcp" + i, tempTexSizeRcp);
                tempTexSizeRcp *= 2.0f;
            }
            ...
            if (!blurMixingPass1DirectionVectorIdx.HasValue || DofPassesChanged(DofAperture) || DofSettingsChanged(DofRotation, aspectRatio, DofAnamorphic))
            {
                ...
                for (int i = 0; i < dofPasses.Length; i++)
                {
                    ...
                    if (dofPasses[i].IsMixingPass)
                    {
                        ...
                        {
                            throw new Exception("Wilberforce Camera Lens depth of field failed - too many mixing passes");
                        }
                    }
                }
            ...
            CameraLensMaterial.SetInt("dofMixTypeMixingPass1", dofMixTypeMixingPass1);
            CameraLensMaterial.SetInt("dofMixTypeMixingPass2", dofMixTypeMixingPass2);
            CameraLensMaterial.SetInt("isDofAuxInputHotPass1", isDofAuxInputHotPass1);
            CameraLensMaterial.SetInt("isDofAuxInputHotPass2", isDofAuxInputHotPass2);

            CameraLensMaterial.SetFloat("dofPower", DofPower);

            if (DofAperture == DofApertureType.Circular)
                CameraLensMaterial.SetFloat("dofSoftness", 0.0f);
            else
                CameraLensMaterial.SetFloat("dofSoftness", (1.0f - DofSoftness) * 4.0f);

            CameraLensMaterial.SetFloat("dofAmount", DofAmount);

            CameraLensMaterial.SetInt("isDofLumaFilter", DofLumaFilterEnabled ? 1 : 0);
            CameraLensMaterial.SetInt("isDofColortint", DofColorTintEnabled ? 1 : 0);
            CameraLensMaterial.SetInt("isDofPushHighlights", DofPushHighlightsEnabled ? 1 : 0);

            CameraLensMaterial.SetFloat("DofLumaThreshold", DofLumaThreshold);
            CameraLensMaterial.SetFloat("DofLumaKneeWidth", DofLumaKneeWidth);
            CameraLensMaterial.SetFloat("DofLumaTwiceKneeWidthRcp", 1.0f / (DofLumaKneeWidth * 2.0f));
            CameraLensMaterial.SetFloat("DofLumaKneeLinearity", DofLumaKneeLinearity);
            CameraLensMaterial.SetColor("dofColorTint", DofColorTint);
            ...
            CameraLensMaterial.SetFloat("dofForegroundMaxCoCDistance", -dofForegroundCoCDistance);
            CameraLensMaterial.SetFloat("dofBackgroundMaxCoCDistance", -dofBackgroundCoCDistance);
            CameraLensMaterial.SetFloat("dofForegroundMaxCoCDiameter", dofForegroundCoCDiameter);
            CameraLensMaterial.SetFloat("dofBackgroundMaxCoCDiameter", dofBackgroundCoCDiameter);

            CameraLensMaterial.SetFloat("dofForegroundCoCLinearity", DofForegroundCoCLinearity);
            CameraLensMaterial.SetFloat("dofBackgroundCoCLinearity", DofBackgroundCoCLinearity);

            CameraLensMaterial.SetInt("isDofThinLensModel", DofCoCModel == DofCoCModelType.RealisticThinLensModel ? 1 : 0);

            SetVectorArrayNoBuffer("separableBlurDirections", CameraLensMaterial, separableBlurDirectionsBuffer);

            if (blurMixingPass1DirectionVectorIdx.HasValue) CameraLensMaterial.SetInt("blurMixingPass1DirectionVectorIdx", blurMixingPass1DirectionVectorIdx.Value);
            if (blurMixingPass2DirectionVectorIdx.HasValue) CameraLensMaterial.SetInt("blurMixingPass2DirectionVectorIdx", blurMixingPass2DirectionVectorIdx.Value);
        ...
        private void SetKeywords(string offState, string onState, bool state)
        {
            if (state)
            {
                CameraLensMaterial.DisableKeyword(offState);
                CameraLensMaterial.EnableKeyword(onState);
            }
            else
            {
                CameraLensMaterial.DisableKeyword(onState);
                CameraLensMaterial.EnableKeyword(offState);
            }
        }
        ...
        private void EnsureMaterials()
        {
            ...
            if (!CameraLensMaterial && cameraLensShader.isSupported)
            {
                CameraLensMaterial = CreateMaterial(cameraLensShader);
            }

            if (!cameraLensShader.isSupported)
            {
        ...
        private void DoShaderBlitCopy(CommandBuffer commandBuffer, ShaderTextureInfo sourceTexture, ShaderTextureInfo destinationTexture)
        {
            ...
            {
                CameraLensMaterial.SetInt("useSPSRFriendlyTransform", 1);
                SetShaderTexture(commandBuffer, "texCopySource", sourceTexture, CameraLensMaterial);
                DoShaderBlit(commandBuffer, sourceTexture, destinationTexture, CameraLensMaterial, (int)ShaderPass.TexCopyImageEffectSPSR);
                CameraLensMaterial.SetInt("useSPSRFriendlyTransform", 0);
            }
            else
            {
        ...
        private void DoShaderBlit(CommandBuffer commandBuffer, ShaderTextureInfo sourceTexture, ShaderTextureInfo destinationTexture, Material material, int passNum)
        {
            if (commandBuffer != null)
            {
                if (material != null)
        ...
        {
            if (ShouldUseCommandBuffer() && (this is CameraLensCommandBuffer) && !(this is CameraLens)) return;
            if (!ShouldUseCommandBuffer() && (this is CameraLens)) return;
            ...
            for (int i = 0; i < allComponents.Length; i++)
            {
                if (ShouldUseCommandBuffer() && (allComponents[i] == this))
                {
                    ...
                    newComponent = oldGameObject.AddComponent<CameraLensCommandBuffer>();
                    (newComponent as CameraLensCommandBuffer).SetParameters(parameters);
                    oldComponentIndex = i;
                    break;
                }
                ...
                if (!ShouldUseCommandBuffer() && ((allComponents[i] == this)))
                {
                    ...
                    TeardownCommandBuffer();
                    DestroyImmediate(this);
                    newComponent = oldGameObject.AddComponent<CameraLens>();
                    (newComponent as CameraLens).SetParameters(parameters);
                    oldComponentIndex = i;
                    break;
                }
            }
        ...
        {
            if (Debug.isDebugBuild) Debug.LogError("Wilberforce Camera Lens Effect Error: " + error);
        }

        private void ReportWarning(string error)
        {
            if (Debug.isDebugBuild) Debug.LogWarning("Wilberforce Camera Lens Effect Warning: " + error);
        }
        ...
        void DoEffect(CommandBuffer commandBuffer, RenderTexture source, RenderTexture destination, int cmdBufferScreenTexture, int screenTextureWidth, int screenTextureHeight)
        {
            ...
            {
                CameraLensMaterial.SetInt("useSPSRFriendlyTransform", 2);
                DoShaderBlit(commandBuffer, sourceTexture, destinationTexture, CameraLensMaterial, (int)ShaderPass.DebugDisplayPass);
                CameraLensMaterial.SetInt("useSPSRFriendlyTransform", 0);
                return;
            }
        ...
        private void ProcessEffect(CommandBuffer commandBuffer, int width, int height, LensEffectType lensEffectType, ShaderTextureInfo sourceTexture, ShaderTextureInfo destinationTexture, RenderTextureFormat screenTextureFormat)
        {
            switch (lensEffectType)
            {
                case LensEffectType.Distortion:
                    ...
                    // Distortion
                    CameraLensMaterial.SetInt("useSPSRFriendlyTransform", 2);
                    DoShaderBlit(commandBuffer, sourceTexture, destinationTexture, CameraLensMaterial, (int)ShaderPass.DistortionOnly);
                    CameraLensMaterial.SetInt("useSPSRFriendlyTransform", 0);
                ...
                case LensEffectType.ChromaticAberrations:
                    ...
                    // Chromatic aberration
                    CameraLensMaterial.SetInt("useSPSRFriendlyTransform", 2);
                    DoShaderBlit(commandBuffer, sourceTexture, destinationTexture, CameraLensMaterial, (int)ShaderPass.ChromaticAberrationOnly);
                    CameraLensMaterial.SetInt("useSPSRFriendlyTransform", 0);
                ...
                case LensEffectType.Vignette:
                    ...
                    // Vignette
                    CameraLensMaterial.SetInt("useSPSRFriendlyTransform", 2);
                    DoShaderBlit(commandBuffer, sourceTexture, destinationTexture, CameraLensMaterial, (int)ShaderPass.VignetteOnly);
                    CameraLensMaterial.SetInt("useSPSRFriendlyTransform", 0);
                ...
                case LensEffectType.DepthOfField:
                    ...
                    SetShaderTexture(commandBuffer, "cocInputTexture", sourceTexture, CameraLensMaterial);
                    CameraLensMaterial.SetInt("useSPSRFriendlyTransform", 1);

                    DoShaderBlitMRT(commandBuffer, sourceTexture, cocPassRTs, CameraLensMaterial, commandBuffer != null ? (int)ShaderPass.CoCMapPassCmdBuffer : (int)ShaderPass.CoCMapPass);
                    ShaderTextureInfo cocFinalTemp = null;
                    ShaderTextureInfo cocHalfTemp = null;

                    SetShaderTexture(commandBuffer, "circleOfConfusionTexture", circleOfConfusionTexture, CameraLensMaterial);
                    CameraLensMaterial.SetInt("useSPSRFriendlyTransform", 0);
                    ...
                    if (DofRange == DofRangeType.ForegroundOnly || DofRange == DofRangeType.ForegroundAndBackground)
                    {
                        ...
                        for (int i = 0; i < cocBlurMipLevels; i++)
                        {
                            ...
                            DoShaderBlit(commandBuffer, sourceTex, downsamplingCocTextures.colorBufferTextures[i], CameraLensMaterial, (int)GetCoCBlurShaderPass(i, isXPass));
                        }
                        ...
                        for (int i = cocBlurMipLevels - 1; i >= 0; i--)
                        {
                            ...
                            DoShaderBlit(commandBuffer, downsamplingCocTextures.colorBufferTextures[i], passDestinationTex, CameraLensMaterial, shaderPass);
                        }

                        if (downsampleCoCBlurInput)
                        {
                            DoShaderBlit(commandBuffer, cocHalfTemp, cocFinalTemp, CameraLensMaterial, (int)ShaderPass.CocBlurUpscale);
                    ...
                    if (cocFinalTemp != null)
                        SetShaderTexture(commandBuffer, "circleOfConfusionTexture", cocFinalTemp, CameraLensMaterial);
                    else
                        SetShaderTexture(commandBuffer, "circleOfConfusionTexture", circleOfConfusionTexture, CameraLensMaterial);

                    SetShaderTexture(commandBuffer, "dofInputTexture", dofInputTexture, CameraLensMaterial);
                    ...
                    if (DofAlgorithm == DofAlgorithmType.Fast)
                    {
                        ...
                        DoShaderBlit(commandBuffer, dofOutputTextureHalf, destinationTexture, CameraLensMaterial, (int)ShaderPass.DepthOfFieldFastMixingPass);
        ...
        private void ResolveActiveEffects()
        {
            ...
            if (lastDistortionEnabled == DistortionEnabled &&
                ...
                lastVignetteOrder == VignetteOrder) return;
        ...
        private void ExecuteBloomBlits(CommandBuffer commandBuffer, ShaderTextureInfo sourceTexture, int width, int height, RenderTextureFormat screenTextureFormat, ShaderTextureInfo destinationTexture)
        {
            ...
            if (BloomAntiflickerEnabled)
            {
                ...
                SetShaderTexture(commandBuffer, "bloomPrepassSource", sourceTexture, CameraLensMaterial);

                SetShaderTexture(commandBuffer, "lumaHistoryBuffer3", lumaHistory[0], CameraLensMaterial);
                if (lumaHistory.Length > 1) SetShaderTexture(commandBuffer, "lumaHistoryBuffer2", lumaHistory[1], CameraLensMaterial);
                if (lumaHistory.Length > 2) SetShaderTexture(commandBuffer, "lumaHistoryBuffer", lumaHistory[2], CameraLensMaterial);

                DoShaderBlitMRT(commandBuffer, sourceTexture, prepassRTs, CameraLensMaterial, (int)(commandBuffer == null ? ShaderPass.BloomPrepassAntiflicker : ShaderPass.BloomPrepassAntiflickerCmdBuffer));

                if (commandBuffer != null)
                {
                    SetShaderTexture(commandBuffer, "texCopySource", lumaHistoryTemp, CameraLensMaterial);
                    commandBuffer.SetRenderTarget(lumaHistory[0]);
                    commandBuffer.DrawMesh(GetScreenQuad(), Matrix4x4.identity, CameraLensMaterial, 0, (int)ShaderPass.TexCopy);
                }
                else
                {
            ...
            {
                DoShaderBlit(commandBuffer, sourceTexture, tempFilterPassTexture, CameraLensMaterial, (int)ShaderPass.BloomPrepass);
            }
            ...
            SetShaderTexture(commandBuffer, "bloomSourceTexture", sourceTexture, CameraLensMaterial);
            CameraLensMaterial.SetInt("useSPSRFriendlyTransform", 2);
            ...
            for (int i = 0; i < bloomDownSamplingLevels; i++)
            {
                ...
                DoShaderBlit(commandBuffer, sourceTex, downsamplingTextures.colorBufferTextures[i], CameraLensMaterial, (int)GetBloomShaderPass(i, isXPass));
            }
            ...
            for (int i = bloomDownSamplingLevels - 1; i >= 0; i--)
            {
                    ...
                    i == bloomDownSamplingLevels - 1)
                {
                    ...
                    // Scaling fix
                    DoShaderBlit(commandBuffer, downsamplingTextures.colorBufferTextures[i], passDestinationTex, CameraLensMaterial, shaderPass);
                }
                else
                {
        ...
        private ShaderPass GetCoCBlurShaderPass(int level, bool isXPass)
        {
            if (isXPass)
            {
                switch (level)
                {
                    ...
                    default: throw new Exception("Wilberforce Camera Lens: invalid coc blur mip level " + level);
                }
            }
            else
            {
        ...
        private ShaderPass GetBloomShaderPass(int level, bool isXPass)
        {
            if (isXPass)
            {
                switch (level)
                {
                    ...
                    default: throw new Exception("Wilberforce Camera Lens: invalid bloom mip level " + level);
                }
            }
            else
            {
        ...
        private void TrySetBloomUniforms(int width, int height)
        {
            ...
            for (int i = 0; i < bloomDownSamplingLevels; i++)
            {
                ...
                {
                    CameraLensMaterial.SetVector("bloomTexSizeRcp" + i, texSizeRcp * bottomLevelRadius);
                    CameraLensMaterial.SetVector("bloomTexSizeRcpBottom", texSizeRcp * bottomLevelRadius);
                }
                else
                {
                    CameraLensMaterial.SetVector("bloomTexSizeRcp" + i, texSizeRcp);
                }
            ...
            CameraLensMaterial.SetFloat("bloomPower", 1.0f / BloomPower);
            CameraLensMaterial.SetFloat("bloomMultiplier", BloomMultiplier);
            CameraLensMaterial.SetFloat("bottomLevelRadius", bottomLevelRadius);
            ...
            CameraLensMaterial.SetFloat("bloomThreshold", BloomThreshold);
            CameraLensMaterial.SetFloat("bloomSaturation", BloomSaturation);
            CameraLensMaterial.SetInt("bloomSize", bloomSamplesCount);
            ...
            SetVectorArray("bloomGauss", CameraLensMaterial, bloomGaussianNormalized, ref bloomGaussianNormalizedBuffer, ref lastBloomGaussianNormalizedBufferLength, bloomGaussianNormalizedNeedUpdate);
        }
        ...
        private void TrySetAntiflickerUniforms()
        {
            ...
            CameraLensMaterial.SetVector("lumaHistoryMask", lumaHistoryMask);
            CameraLensMaterial.SetVector("lumaHistoryWeights3", lumaHistoryWeights[0]);
            if (historyFramesNeeded > 1) CameraLensMaterial.SetVector("lumaHistoryWeights2", lumaHistoryWeights[1]);
            if (historyFramesNeeded > 2) CameraLensMaterial.SetVector("lumaHistoryWeights", lumaHistoryWeights[2]);
            CameraLensMaterial.SetFloat("currentLumaWeight", currentLumaWeight);
            CameraLensMaterial.SetInt("lumaHistoryLength", historyFramesNeeded);

            CameraLensMaterial.SetInt("bloomAntiflickerFade", (int)BloomAntiflickerFade);
        ...
        private void ExecuteDofImageEffectBlits(CommandBuffer commandBuffer, DofPass[] dofPasses, int width, int height, RenderTextureFormat screenTextureFormat, ShaderTextureInfo cocTexture, ShaderTextureInfo destinationTexture)
        {
            if (dofPasses != null)
            {
                ...
                for (int i = 0; i < dofPasses.Length; i++)
                {
                    ...
                    {
                        SetShaderTexture(commandBuffer, "bokehAuxTexture" + nextBokehAuxTextureNum, auxTexture, CameraLensMaterial);
                    ...
                    DoShaderBlit(commandBuffer, sourceTexture, targetTexture, CameraLensMaterial, (int)dofShaderPassToUse);
                }
...
#region Distortion
    ...
    [CustomEditor(typeof(CameraLensCommandBuffer))]
    public class CameraLensEditorCmdBuffer : CameraLensEditor { }

    public class CameraLensEditor : Editor
    {
...
#region Labels
        ...
        private readonly GUIContent chromaRadialnessLabel = new GUIContent("Linearity: ", "Size based on distance from image center (makes aberrations larger in corners).");
        ...
        private CameraLens.DofCoCFromParametersModeType lastDofCoCFromParametersMode;
        ...
        private CameraLens.DofCoCModelType lastDofCoCModel;
        ...
        private CameraLens.FocalLengthSourceType lastFocalLengthSource;
        ...
        private CameraLens.VignetteModeType lastVignetteMode;

#endregion

#region Foldouts

        private bool orderFoldout = false;
        private bool pipelineFoldout = false;
        private bool aboutFoldout = false;
        private bool bloomFoldout = false;
        private bool vignetteFoldout = false;
        private bool bokehFoldout = true;
        private bool chromaAbrrFoldout = false;
        private bool distortionFoldout = false;
        private bool lensModelFoldout = false;
...
#region Graph Widgets Params
        ...
        private GraphWidgetDrawingParameters GetDofLumaGraphWidgetParameters(CameraLensCommandBuffer script)
        {
        ...
        private GraphWidgetDrawingParameters GetDofCocGraphWidgetParameters(CameraLensCommandBuffer script)
        {
            ...
            if (script.FocalLengthSource == CameraLens.FocalLengthSourceType.CameraFOV)
                focalLengthToUse = GetFocalLengthFromVerticalFoV(camera.fieldOfView, script.SensorHeight * 0.001f);

            if (script.DofCoCFromParametersMode == CameraLens.DofCoCFromParametersModeType.RelativeToFocalPlane)
            {
            ...
            cocGraphWidgetParams = new GraphWidgetDrawingParameters()
            {
                ...
                GraphFunction = ((float viewSpaceDepth) =>
                {
                    ...
                    if (lastDofCoCModel == CameraLens.DofCoCModelType.FromParameters)
                    {
            ...
            if (lastDofCoCModel != CameraLens.DofCoCModelType.RealisticThinLensModel)
            {
        ...
        private GraphWidgetDrawingParameters GetVignetteGraphWidgetParameters(CameraLensCommandBuffer script)
        {
            ...
            vignetteWidgetParams = new GraphWidgetDrawingParameters()
            {
                ...
                GraphFunction = ((float distance) =>
                {
                    ...
                    if (script.VignetteMode == CameraLens.VignetteModeType.Standard)
                    {
                        u = Mathf.Lerp(script.VignetteOuterValue, script.VignetteInnerValue, u);
                    }
                    else if (script.VignetteMode == CameraLens.VignetteModeType.Saturation)
                    {
...
#region Unity Utilities
        ...
        private void SetIcon()
        {
            ...
            {
                if (Debug.isDebugBuild) Debug.Log("Wilberforce Camera Lens Effect Error: There was an exception while setting icon to Wilberforce Camera Lens script: " + ex.Message);
            }
        }
        ...
        void OnEnable()
        {
            ...
            CameraLensCommandBuffer effect = (target as CameraLensCommandBuffer);
            ...
            {
                if (Debug.isDebugBuild) Debug.Log("Wilberforce Camera Lens Effect Error: There was an exception while loading icons for Wilberforce Camera Lens script: " + ex.Message);
            }
        }
        ...
        void DisplayGBufferStateMessage(CameraLensCommandBuffer script)
        {
        ...
        {
            var script = target as CameraLensCommandBuffer;
            ...
            if (pipelineFoldout)
            {
                script.IntegrationType = (CameraLens.Integration)EditorGUILayout.EnumPopup(integrationLabelContent, script.IntegrationType);
                EditorGUILayout.Space();

                script.FarPlaneSource = (CameraLensCommandBuffer.FarPlaneSourceType)EditorGUILayout.EnumPopup(farPlaneSourceLabelContent, script.FarPlaneSource);

                //script.IntegrationStage = (CameraLens.CameraEventType)EditorGUILayout.EnumPopup(integrationLabelContent, script.IntegrationStage);
            ...
            CameraLens.BloomDofOrderType bloomDofOrder = script.BloomOrder < script.DofOrder ? CameraLens.BloomDofOrderType.BloomBeforeDepthOfField : CameraLens.BloomDofOrderType.DepthOfFieldBeforeBloom;
            CameraLens.VignetteOrderType vignetteOrder = script.BloomOrder < script.VignetteOrder ? CameraLens.VignetteOrderType.VignetteAfterBloom : CameraLens.VignetteOrderType.VignetteBeforeBloom;
            CameraLens.DistortionOrderType distortionOrder = script.DistortionOrder < script.DofOrder ? CameraLens.DistortionOrderType.DistortionBeforeDepthOfField : CameraLens.DistortionOrderType.DistortionAfterDepthOfField;
            var newBloomDofOrder = bloomDofOrder;
            var newVignetteOrder = vignetteOrder;
            var newDistortionOrder = distortionOrder;

            orderFoldout = EditorGUI.Foldout(EditorGUILayout.GetControlRect(), orderFoldout, "Effects Order Settings: ", true, EditorStyles.foldout);
            if (orderFoldout)
            {
                EditorGUI.indentLevel++;
                EditorGUILayout.Space();

                newBloomDofOrder = (CameraLens.BloomDofOrderType)EditorGUILayout.EnumPopup("Bloom & Depth of Field Order: ", bloomDofOrder);
                newVignetteOrder = (CameraLens.VignetteOrderType)EditorGUILayout.EnumPopup("Vignette Order: ", vignetteOrder);
                newDistortionOrder = (CameraLens.DistortionOrderType)EditorGUILayout.EnumPopup("Distortion Order: ", distortionOrder);
                EditorGUI.indentLevel--;
            }
            ...
            {
                if (newBloomDofOrder == CameraLens.BloomDofOrderType.BloomBeforeDepthOfField)
                {
                    if (newVignetteOrder == CameraLens.VignetteOrderType.VignetteBeforeBloom)
                    {
                        if (newDistortionOrder == CameraLens.DistortionOrderType.DistortionBeforeDepthOfField)
                        {
            ...
            Undo.RecordObject(target, "Wilberforce Camera Lens change");
        }

        private void DistortionGUI(CameraLensCommandBuffer script)
        {
            HeaderWithToggle("Lens Distortion", ref distortionFoldout, ref script.DistortionEnabled, distortionIcon);
        ...
        private void ChromaticAbbrGUI(CameraLensCommandBuffer script)
        {
            HeaderWithToggle("Chromatic Aberrations", ref chromaAbrrFoldout, ref script.ChromaEnabled, chromaAbrrIcon);

            if (chromaAbrrFoldout)
            {
                ...
                script.ChromaQuality = (CameraLens.Quality)EditorGUILayout.EnumPopup("Quality: ", script.ChromaQuality);
        ...
        private void BokehGUI(CameraLensCommandBuffer script)
        {

            HeaderWithToggle("Depth of Field (Bokeh)", ref bokehFoldout, ref script.DofEnabled, bokehIcon);
            ...
            if (bokehFoldout)
            {
                ...
                script.DofAlgorithm = (CameraLens.DofAlgorithmType)EditorGUILayout.EnumPopup(dofAlgorithmTypeLabel, script.DofAlgorithm);
                ...
                EditorGUI.BeginDisabledGroup(script.DofCoCModel != CameraLens.DofCoCModelType.RealisticThinLensModel);
                script.ApertureDiameter = EditorGUILayout.Slider(apertureDiameterLabel, script.ApertureDiameter, 1.0f, 250.0f);
                EditorGUI.EndDisabledGroup();

                EditorGUI.BeginDisabledGroup(script.DofAperture == CameraLens.DofApertureType.Circular);
                script.DofSoftness = EditorGUILayout.Slider(dofSoftnessLabel, script.DofSoftness, 0.0f, 1.0f);
                EditorGUI.EndDisabledGroup();

                EditorGUILayout.Space();

                script.DofAperture = (CameraLens.DofApertureType)EditorGUILayout.EnumPopup(apertureShapeLabel, script.DofAperture);
                script.DofRotation = EditorGUILayout.Slider(apertureRotationLabel, script.DofRotation, 0, 360.0f);

                EditorGUILayout.Space();

                script.DofRange = (CameraLens.DofRangeType)EditorGUILayout.EnumPopup(dofRangeLabel, script.DofRange);
                EditorGUILayout.Space();

                script.DofQuality = (CameraLens.Quality)EditorGUILayout.EnumPopup("Quality: ", script.DofQuality);
                ...
                lensModelFoldout = EditorGUI.Foldout(EditorGUILayout.GetControlRect(), lensModelFoldout, lensModelLabelContent, true, EditorStyles.foldout);
                if (lensModelFoldout)
                {

                    script.DofCoCModel = (CameraLens.DofCoCModelType)EditorGUILayout.EnumPopup(cocModelLabelContent, script.DofCoCModel);

                    if (script.DofCoCModel == CameraLens.DofCoCModelType.FromParameters)
                    {
                        EditorGUILayout.Space();

                        script.DofCoCFromParametersMode = (CameraLens.DofCoCFromParametersModeType)EditorGUILayout.EnumPopup(cocParametersUnitsLabelContent, script.DofCoCFromParametersMode);
                        EditorGUILayout.Space();
                    ...
                    else
                    {
                        ...
                        script.FocalLengthSource = (CameraLens.FocalLengthSourceType)EditorGUILayout.EnumPopup(focalLengthSourceLabelContent, script.FocalLengthSource);

                        if (script.FocalLengthSource == CameraLens.FocalLengthSourceType.Manual)
                        {
                ...
                script.DofAnamorphic = (CameraLens.DofAnamorphicType) EditorGUILayout.EnumPopup(anamorphicLensLabel, script.DofAnamorphic);
        ...
        private void BloomGUI(CameraLensCommandBuffer script)
        {
            HeaderWithToggle("Bloom", ref bloomFoldout, ref script.BloomEnabled, bloomIcon);
            ...
            if (bloomFoldout)
            {
                ...
                script.BloomQuality = (CameraLens.Quality)EditorGUILayout.EnumPopup(bloomQualityLabelContent, script.BloomQuality);
                EditorGUILayout.Space();
                ...
                if (script.BloomAntiflickerEnabled)
                {
                    ...
                    script.BloomAntiflickerLength = (CameraLens.LengthType)EditorGUILayout.EnumPopup(antiflickerLengthLabelContent, script.BloomAntiflickerLength);

                    if (script.BloomAntiflickerLength != CameraLensCommandBuffer.LengthType.Short &&
                        script.IntegrationType == CameraLensCommandBuffer.Integration.CommandBuffer)
                    {
                        EditorGUILayout.HelpBox("Only 'Short' length of antiflicker filter is available in 'command buffer' mode.", MessageType.Warning);
                    }

                    script.BloomAntiflickerFade = (CameraLens.FadeType)EditorGUILayout.EnumPopup(antiflickerFadeLabelContent, script.BloomAntiflickerFade);
        ...
        //private void FilmGUI(CameraLens script)
        //{
        //    HeaderWithToggle("Film & Grain", ref filmFoldout, ref script.FilmNGrainEnabled, filmIcon);

        //    if (filmFoldout)
        //    {
        //        EditorGUI.BeginDisabledGroup(!script.FilmNGrainEnabled);
        //        EditorGUI.indentLevel++;
        ...
        private void VignetteGUI(CameraLensCommandBuffer script)
        {
            HeaderWithToggle("Vignette", ref vignetteFoldout, ref script.VignetteEnabled, vignetteIcon);
            ...
            if (vignetteFoldout)
            {
                ...
                EditorGUILayout.Space();
                script.VignetteMode = (CameraLens.VignetteModeType)EditorGUILayout.EnumPopup(vignetteModeLabelContent, script.VignetteMode);

                if (script.VignetteMode == CameraLens.VignetteModeType.CustomColors)
                {
                    EditorGUI.indentLevel++;
